...
std::string menu[10];
menu[0]="Âûáåğèòå êàáåëü:";


std::string cables[20];
cables[0]="ÍÊÊ-ÌËÈ";
cables[1]="ÍÊÊ-Ïëàòà ïåğåêë";
cables[2]="ÏÏ1-ÏÏ2";
cables[3]="ÏÊÓ-ÊÌ(Àëèñà)";
cables[4]="ÏÊÓ-ÊÌ(Áîá)";
cables[5]="ÏÊÓ-SD/CS";
cables[6]="ÍÊÊ-êîìïåíñ.äèñï.";
cables[7]="ÏÊÓ-ÍÊÊ(UART)";
cables[8]="ÏÊÓ-ÍÊÊ(ÀÎÌ)";
cables[9]="ÏÊÓ-ÍÊÊ-ÄÎÔ-ÍÊÊ";
cables[10]="ÏÊÓ-ÍÊÊ-ÄÎÔ-ÄÎÔ";
cables[11]="ÏÊÓ-ÍÊÊ-ÀÎÌ";
cables[12]="internal frdge";
cables[13]="ÏÊÓ-ÍÊÊ-ÀÎÌ_v4";
cables[14]="Ehernet";
cables[15]="ÍÊÊ-ÎÏ";
...
*/
  ...
  RCC->APB2ENR|=RCC_APB2ENR_ADC1EN; //Âêëş÷àåì òàêòèğîâàíèå ÀÖÏ
  ADC1->CR2 |= ADC_CR2_CAL; //Çàïóñê êàëèáğîâêè ÀÖÏ
  while (!(ADC1->CR2 & ADC_CR2_CAL))
    ; //Îæèäàåì îêîí÷àíèÿ êàëèáğîâêè
  ADC1->SMPR2 |= (ADC_SMPR2_SMP1_2 | ADC_SMPR2_SMP1_1 | ADC_SMPR2_SMP1_0); //Çàäàåì
                                                                                            // äëèòåëüíîñòü âûáîğêè
  ADC1->CR2 |= ADC_CR2_JEXTSEL; //Ïğåîáğàçîâàíèå èíæåêòèğîâàííîé ãğóïïû
                                                       //çàïóñòèòñÿ óñòàíîâêîé áèòà JSWSTART
  ADC1->CR2 |= ADC_CR2_JEXTTRIG; //Ğàçğåøàåì âíåøíèé çàïóñê èíæåêòèğîâàííîé ãğóïïû
  ADC1->CR2 |= ADC_CR2_CONT; //Ïğåîáğàçîâàíèÿ çàïóñêàşòñÿ îäíî çà äğóãèì
  ADC1->CR1 |= ADC_CR1_JAUTO; //Ğàçğåøèòü ïğåîáğàçîâàíèå èíæåêòèğîâàííîé ãğóïïû
                                     //ïîñëå ğåãóëÿğíîé. Íå ïîíÿòíî çà÷åì, íî áåç ıòîãî íå ğàáîòàåò
  ADC1->JSQR |= (10<<15); //Çàäàåì íîìåğ êàíàëà (âûáğàí ADC1)
  ADC1->CR2 |= ADC_CR2_ADON;//Òåïåğü âêëş÷àåì ÀÖÏ
  ADC1->CR2 |= ADC_CR2_JSWSTART; //Çàïóñê ïğåîáğàçîâàíèé
  while (!(ADC1->SR & ADC_SR_JEOC)) //æäåì ïîêà ïåğâîå ïğåîáğàçîâàíèå çàâåğøèòñÿ
    ;
  //Òåïåğü ìîæíî ÷èòàòü ğåçóëüòàò èç JDR1
  uint32_t adc_res; //Èñïîëüçîâàë ïåğåìåííóş äëÿ îòëàäêè. Ìîæíî è áåç íå¸
